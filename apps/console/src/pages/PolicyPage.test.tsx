...
describe("PolicyPage", () => {
  it("supports accordion module editing for semantic/genai fields", async () => {
    const user = userEvent.setup();
    render(<PolicyPage />);

    await screen.findByText("Policy editor");

    const sensitivityField = await screen.findByLabelText("Sensitivity labels (comma-separated)");
    await user.clear(sensitivityField);
    await user.type(sensitivityField, "Public, Restricted");
    expect(sensitivityField).toHaveValue("Public, Restricted");

    await user.click(screen.getByRole("button", { name: /Semantic DLP semantic_dlp/i }));
    await waitFor(() => {
      expect(screen.queryByLabelText("Sensitivity labels (comma-separated)")).toBeNull();
    });

    await user.click(screen.getByRole("button", { name: /Semantic DLP semantic_dlp/i }));
    expect(await screen.findByLabelText("Sensitivity labels (comma-separated)")).toBeInTheDocument();
  });

  it("runs simulation from selected policy and shows gate summary", async () => {
    const user = userEvent.setup();
    render(<PolicyPage />);

    await screen.findByText("Policy list");
    ...
    expect(await screen.findByText(/Approval gate active/i)).toBeInTheDocument();
  });

  it("shows assignment modal with inheritance preview for selected company", async () => {
    const user = userEvent.setup();
    render(<PolicyPage />);
    ...
    expect(await screen.findByRole("heading", { name: "Assign policy" })).toBeInTheDocument();

    await user.selectOptions(screen.getByLabelText("Company"), "customer-1");

    await waitFor(() => {
      expect(apiGetMock).toHaveBeenCalledWith("/policies/effective?customer_id=customer-1");
    });
    expect(await screen.findByText("Assignments applied: 1")).toBeInTheDocument();
  });
});
