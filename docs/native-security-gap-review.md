...
| Web protection | GenAI destination detection in DLP path; browser sensor planned | Managed browser extension, URL/domain reputation, anti-phishing, SSL/TLS inspection strategy, upload/download enforcement, Search Advisor equivalent if desired |
...
| Content and device control | Deterministic DLP scan API, policy documents, policy simulation, labels planned | Endpoint clipboard/file/upload/email/USB/print/screenshot enforcement, app/website policy, device inventory, label propagation, exception workflow |
...
| Web protection, anti-phishing, safe browsing | DLP can detect GenAI sink context for submitted text; browser sensor is planned. | Browser extension or managed browser sensor for URL reputation, phishing detection, upload/paste inspection, and destination-aware DLP. Decide whether SSL inspection is in scope or replaced with endpoint/browser event inspection. | Avoid becoming a network MITM product in v1 unless certificate management and privacy controls are designed. |
